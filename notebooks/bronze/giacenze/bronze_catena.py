...
# Versione: 1.0.0
# Data: 2026-06-09
# Descrizione: Ingestion 1:1 della sorgente RAW logistix CATENA verso Bronze Delta.
#              Sorgente: CATENA @LOG (multi-sito via db-link) -> landing logistix-landing/{sito}/catena.
#              MODE = SNAPSHOT (catasta giacenze, fotografia giornaliera).
#              Schema-on-read (StringType), SELECT colonne sorgente + metadati _bronze_*.
#              Bronze 1:1 PURO: NIENTE derivate (no mag_sito_cod, no ART_RADICE/VAR) -> Silver.
#              Multi-sito: union dei siti come bronze_struttura_mag (decisione catalogo §11, scelta B).
#              Riferimento: Revisione AS-IS to-be - Migrazione CDT_ESTR.md §11 (Catalogo RAW, riga WL1_CATENA).
...
# COMMAND ----------
# MAGIC %md #### 3. Costruzione path landing (logistix-landing/{sito}/catena)
...
bronze_df = (
    ...
    # _sito_estrazione = metadato tecnico (db-link/sito), NON dato di business
    .withColumn("_sito_estrazione", F.regexp_extract(F.input_file_name(), r"/logistix-landing/([^/]+)/", 1))
)
...
# SNAPSHOT idempotente: dynamic partition overwrite per _bronze_load_date.
# Ri-eseguire lo stesso run_date sovrascrive SOLO quella partizione (no raddoppio), storico intatto.
