...
# Versione: 1.0.0
# Data: 2026-06-09
# Descrizione: Ingestion 1:1 della sorgente RAW vettori@TRACK (db-link @TRACK).
#              Sorgente landing: track-landing/vettori (blocco config 'track').
#              MODE = FULL_OVERWRITE (anagrafica).
#              Schema-on-read (StringType). SELECT * 1:1, NESSUNA derivata, NESSUN join.
#              È la fonte che il legacy V_VETTORI usa via WL1_VETTORI_TRASPO (@TRACK):
#              fonte AUTORITATIVA per T_VETTORI (vedi silver_t_vettori).
#              Riferimento: Revisione AS-IS to-be §11 (Trasporti, WL1_VETTORI_TRASPO @TRACK).
...
NOTEBOOK_NAME   = "bronze_vettori_track"
SOURCE_SYSTEM   = "track"
LANDING_SUBDIR  = "track-landing"
TABLE_NAME      = "vettori"
TARGET_NAME     = "vettori_track"   # disambigua dal vettori locale
MODE            = "FULL_OVERWRITE"
...
bronze_df = (
    ...
    .withColumn("_sito_estrazione", F.lit(SOURCE_SYSTEM))  # db-link @TRACK: metadato tecnico
)
