...
# Versione: 3.0.0
# Data: 2026-06-08
# Descrizione: Bronze PESATE — MODE=DELTA_MERGE.
...
NOTEBOOK_NAME = "bronze_pesate"
SOURCE_SYSTEM = "logistix"
TABLE_NAME    = "pesate"
MODE          = "DELTA_MERGE"
IS_MULTISITE  = SOURCE_SYSTEM == "logistix"

MERGE_KEYS    = ["MAG_SITO_COD", "PSP_NUMETIC", "PSP_DATABOLLA"]
...
source_schema = StructType([
    ...
    StructField("PSP_CNUF",               StringType(), True),
    StructField("PSP_DEPT",               StringType(), True),
    StructField("PSP_DREC",               StringType(), True),
    StructField("PSP_NREC",               StringType(), True),
    StructField("PSP_SITO",               StringType(), True),
    StructField("PSP_NRCARLOG",           StringType(), True),
    StructField("PSP_NUMBOL",             StringType(), True),
    StructField("PSP_DATABOLLA",          StringType(), True),
    StructField("PSP_NCOM",               StringType(), True),
    StructField("PSP_ARTEAN13",           StringType(), True),
    StructField("PSP_NUMETIC",            StringType(), True),
    StructField("PSP_PESOLORDO",          StringType(), True),
    StructField("PSP_CODCAUZPRINC",       StringType(), True),
    StructField("PSP_TARACAUZPRINC",      StringType(), True),
    StructField("PSP_CODCAUZSECOND",      StringType(), True),
    StructField("PSP_TARACAUZSECOND",     StringType(), True),
    StructField("PSP_NRCOLLI",            StringType(), True),
    StructField("PSP_PZXCART",            StringType(), True),
    StructField("PSP_TRASFERITO",         StringType(), True),
    StructField("PSP_CODCAUZFORN",        StringType(), True),
    StructField("PSP_CODVUOADD",          StringType(), True),
    StructField("PSP_DATA_SCADENZA",      StringType(), True),
    StructField("PSP_LOCAZIONE",          StringType(), True),
    StructField("PSP_QTA_UF_RIL",         StringType(), True),
    StructField("PSP_PESOMEDIO",          StringType(), True),
    StructField("PSP_DATAINS",            StringType(), True),
    StructField("PSP_DATA_ESTRAZIONE_DWH",StringType(), True),
    StructField("DATA_BOLLA_NUM",         StringType(), True),
    StructField("DATA_CARICO_RIGHE",      StringType(), True),
])
